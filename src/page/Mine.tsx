  ...
  const handleBoost = () => {
    if (tab == 'time' && setting.dailyTimeLimitList[timeLimit.id]) {
      if (totalPoint >= setting.dailyTimeLimitList[timeLimit.id].coinsToBoost) {
        ...
        let plusCountDown =
          (setting.dailyTimeLimitList[timeLimit.id].value -
            setting.dailyTimeLimitList[timeLimit.id - 1].value) *
          60
        axios
          .put(`${ENDPOINT}/api/user/timeLimit/${user?.id}`, {
            newTimeLimit,
            fee,
            plusCountDown
          })
          .then(res => {
            if (res.data) {
              ...
              let newPoint =
                totalPoint -
                setting.dailyTimeLimitList[timeLimit.id].coinsToBoost
      ...
      if (totalPoint >= setting.powerList[power.id].coinsToBoost) {
        axios
          .put(`${ENDPOINT}/api/user/power/${user?.id}`, {
            newPower: setting.powerList[power.id],
            fee: setting.powerList[power.id].coinsToBoost
          })
          .then(res => {
            if (res.data) {
              toast.success('Boosted successfully!')
              let newPoint =
                totalPoint - setting.powerList[power.id].coinsToBoost
              setTotalPoint(newPoint)
    ...
    <div className='pb-[40px]'>
      <div className='w-full flex flex-col text-center items-center py-2 gap-4'>
        ...
        <div className='flex flex-row items-center justify-center'>
          ...
          <div className='flex flex-col balance gap-2 w-full'>
            <h1 className='font-bold text-[30px] text-white'>
              &nbsp;{formatNumberWithCommas(totalPoint)}
            </h1>
          </div>
        </div>
        ...
        <div className='flex flex-row w-full items-center justify-between gap-4 text-[16px] font-extrabold'>
          ...
          >
            <div
              className={`customCard flex flex-col items-center justify-center group w-full py-2 transition relative duration-300 cursor-pointer hover:translate-y-[3px] hover:bg-inherit ${
                tab == 'time' && 'bg-inherit'
        ...
        <div className='w-full space-y-2'>
          ...
          <ProgressBar
            value={
              ((totalPoint - level.coinsToLevelUp) /
                (nextLevel.coinsToLevelUp - level.coinsToLevelUp)) *
              100
            }
          />
        </div>
        <div className='customCard-container w-full'>
          <div
            onClick={handleBoost}
            className='customCard bg-[#023744] group w-full py-2 transition relative duration-300 cursor-pointer hover:translate-y-[3px] hover:bg-inherit hover:shadow-[0 -8px 0px 0px #2196f3]'
          >
            {tab == 'time' && (
              <div className='flex flex-row items-center justify-between p-2'>
                ...
                <h2 className='text-[11px] text-[#FFF8E1]'>
                  Upgrade your daily mining Time
                </h2>
              </div>
            )}
            {tab == 'power' && (
                ...
                <div className='flex flex-row items-center justify-center gap-2'>
                    ...
                    (setting?.dailyTimeLimitList[timeLimit.id] ? (
                      <>
                        <h2 className='text-[11px] font-bold text-white'>
                          Upgrade to{' '}
                          {setting?.dailyTimeLimitList[timeLimit.id].value} Min{' '}
                        </h2>
                        <h2 className='text-[11px] font-bold text-white'>
                          Cost{' '}
                          {
                  ...
                  {tab == 'power' &&
                    (setting?.powerList[power.id] ? (
                      <>
                        <h2 className='text-[11px] font-bold text-white'>
                          Upgrade to {setting?.powerList[power.id].value}MH/s
                        </h2>
                        <h2 className='text-[11px] font-bold text-white'>
                          Cost {setting?.powerList[power.id].coinsToBoost} $
                          Buffy
                        </h2>
                      </>
                    ) : (
